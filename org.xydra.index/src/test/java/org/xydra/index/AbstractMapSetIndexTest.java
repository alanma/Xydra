...
public abstract class AbstractMapSetIndexTest<K, E> {
	...
	public void testTupleIterator() {
		...
		list = Iterators.toList(this.mapSetIndex
				.tupleIterator(new Wildcard<K>(), new Wildcard<E>()));
		assertEquals(3, list.size());

		list = Iterators.toList(this.mapSetIndex.tupleIterator(new EqualsConstraint<K>(this.key1),
				new Wildcard<E>()));
		assertEquals(2, list.size());

		list = Iterators.toList(this.mapSetIndex.tupleIterator(new EqualsConstraint<K>(this.key2),
				new Wildcard<E>()));
		assertEquals(1, list.size());

		list = Iterators.toList(this.mapSetIndex.tupleIterator(new EqualsConstraint<K>(this.key3),
				new Wildcard<E>()));
