...
public abstract class AbstractMapIndexTest<K, E> {
	...
	public void testTupleIterator() {
		...
		list = Iterators.toList(this.mapIndex.tupleIterator(new EqualsConstraint<K>(this.key1)));
		assertEquals("list=" + list, 1, list.size());
		assertTrue(list.get(0).getEntry() == this.entry3);

		list = Iterators.toList(this.mapIndex.tupleIterator(new EqualsConstraint<K>(this.key2)));
		assertEquals(1, list.size());
		assertTrue(list.get(0).getEntry() == this.entry2);

		list = Iterators.toList(this.mapIndex.tupleIterator(new EqualsConstraint<K>(this.key3)));
