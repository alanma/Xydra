...
public class FastTripleMap<K, L, M, E> implements IMapMapMapIndex<K, L, M, E> {
	...
	public Iterator<KeyKeyKeyEntryTuple<K, L, M, E>> tupleIterator(Constraint<K> c1,
		...
		if (c2.isStar() ? c3.isStar() : !c1.isStar()) {
			final Iterator<KeyKeyKeyEntryTuple<K, L, M, E>> it = this.idx_k1_k2_k3.tupleIterator(
					c1, c2, c3);
		...
		if (!c2.isStar()) {
			final Iterator<KeyKeyKeyEntryTuple<L, M, K, E>> it = this.idx_k2_k3_k1.tupleIterator(
					c2, c3, c1);
			return new Iterator<KeyKeyKeyEntryTuple<K, L, M, E>>() {
		...
		final Iterator<KeyKeyKeyEntryTuple<M, K, L, E>> it = this.idx_k3_k1_k2.tupleIterator(c3,
				c1, c2);
		return new Iterator<KeyKeyKeyEntryTuple<K, L, M, E>>() {
			...
			public KeyKeyKeyEntryTuple<K, L, M, E> next() {
				this.last = it.next();
				return new KeyKeyKeyEntryTuple<K, L, M, E>(this.last.getKey2(),
						this.last.getKey3(), this.last.getKey1(), this.last.getEntry());
			}
