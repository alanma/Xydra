...
public class AddressWidget extends Composite {
    ...
    private static void openAddress(final XAddress desiredAddress) {
        ...
        // }
        ViewModel.getInstance().openLocation(desiredAddress);
        Controller.getInstance().fetchModelIds(repoAddress, new CompoundActionCallback() {
            
            @Override
            public void presentModelAndContinue() {
                Controller.getInstance().presentModel(desiredAddress);
                Controller.getInstance().loadCurrentModelsObjects(this);
            }
            
            @Override
            public void presentObjects() {
                Controller.getInstance().notifyTableController(desiredAddress,
                        TableController.Status.Opened);
                Controller.getInstance().notifyTableController(desiredAddress,
                        TableController.Status.Opened);
                Controller.getInstance().getTableController().scrollToField(desiredAddress);
            }
        });
    }
