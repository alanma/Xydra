...
public class FieldWidget extends Composite {
	...
	public FieldWidget(final XAddress currentFieldAddress, XValue fieldValue, long revisionNumber) {
		...
		this.address = currentFieldAddress;
		
		ValueType valueType = fieldValue.getType();
		this.valueInfoLabel.setText(valueType.toString());
		
		this.revisionNumberLabel.setText("rev.: " + revisionNumber);
		
		XValueEditor editor = getEditorForType(valueType, fieldValue, currentFieldAddress);
		
		this.valuePanel.add(editor);
		
	}
	
	@UiHandler("deleteFieldButton")
	void onClick(ClickEvent event) {
		RemoveElementDialog removeDialog = new RemoveElementDialog(this.address);
		removeDialog.show();
