...
public class Post extends JavaScriptObject {
	...
	public final Date getCreatedTimeDate() {
		return DateTimeFormat.getFormat("yyyy-MM-dd'T'hh:mm:ssZ").parse(getCreatedTime());
	}
	
	public final native String getUpdatedTime() /*-{ return this.updated_time;  }-*/;
	
	public final Date getUpdatedTimeDate() {
		return DateTimeFormat.getFormat("yyyy-MM-dd'T'hh:mm:ssZ").parse(getUpdatedTime());
	}
	...
	public final HTML toHTML() {
		String h = "Id: " + getId() + "<br/>" + "Message: " + getMessage();
		return new HTML(h);
	}
