...
public class MemStateModel extends MemStateEntity implements XStateWritableModel {
    ...
    public MemStateModel(XAddress address) {
        this(address, new HashMap<XId,XStateWritableObject>(2));
    }
    
    public MemStateModel(XAddress address, Map<XId,XStateWritableObject> objects) {
        super(address);
        assert address.getAddressedType() == XType.XMODEL : address;
        this.objects = objects;
    }
    
    private void addObject(@NeverNull XStateWritableObject object) {
    ...
    @Override
    public XStateWritableObject createObject(@NeverNull XId objectId) {
        XStateWritableObject object = this.objects.get(objectId);
        if(object != null) {
            return object;
        }
        XStateWritableObject newObject = new MemStateObject(Base.resolveObject(getAddress(),
                objectId));
    ...
    @Override
    public XStateWritableObject getObject(@NeverNull XId objectId) {
    ...
    public boolean removeObject(@NeverNull XId objectId) {
        XStateWritableObject oldObject = this.objects.remove(objectId);
    ...
    public String toString() {
        return this.getAddress() + " , " + this.objects.size() + " objects";
    }
     ...
     */
    public static XStateWritableModel shallowCopy(XStateWritableModel model) {
        if(model == null) {
            return null;
        }
        
        MemStateModel result = new MemStateModel(model.getAddress());
