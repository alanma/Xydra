 ...
 * 
 */
public interface XReadableRepository extends IHasXAddress, IHasXID, Iterable<XID> {
	 ...
	 *            returned
	 * @return the {@link XReadableModel} with the given {@link XID} or null if no
	 *         such {@link XReadableModel} exists in this repository.
	 */
	@ReadOperation
	XReadableModel getModel(XID id);
	
	/**
	 * Checks whether this repository contains an {@link XReadableModel} with the
	 * given {@link XID}.
	 * 
	 * @param id The {@link XID} which is to be checked
	 * @return true, if this repository contains an {@link XReadableModel} with the
