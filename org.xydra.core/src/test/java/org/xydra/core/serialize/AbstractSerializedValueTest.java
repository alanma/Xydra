...
abstract public class AbstractSerializedValueTest extends AbstractSerializingTest {
	...
	public void testAddressListValue() {
		testValue(XV.toValue(new XAddress[] { Base.toAddress(Base.createUniqueId(), null, null, null),
		        Base.toAddress("/cookie/monster/-/-") }));
	}
	...
	public void testAddressSetValue() {
		testValue(XV.toAddressSetValue(new XAddress[] {
		        Base.toAddress(Base.createUniqueId(), null, null, null),
		        Base.toAddress("/cookie/monster/-/-") }));
	}
	...
	public void testAddressValue() {
		testValue(Base.toAddress(Base.createUniqueId(), null, null, null));
	}
	...
	public void testBinaryValue() {
		final byte[] bytes = new byte[2 * 256];
		for(int i = 0; i < 256; i++) {
			bytes[i] = (byte)i;
	...
	public void testIdListValue() {
		testValue(XV.toValue(new XId[] { Base.createUniqueId(), Base.toId("cookie") }));
	}
	...
	public void testIdSetValue() {
		testValue(XV.toIdSetValue(new XId[] { Base.createUniqueId(), Base.toId("cookie") }));
	}
	...
	public void testIdSortedSetValue() {
		testValue(XV.toIdSortedSetValue(new XId[] { Base.createUniqueId(), Base.toId("cookie") }));
	}
	...
	private void testValue(final XValue value) {
