...
public class TransactionTest {
    ...
    @Before
    public void setUp() {
        this.actorId = Base.toId("AbstractTransactionTest");
        ...
        final XField johnsPhone = this.john.createField(PHONE_ID);
        johnsPhone.setValue(JOHN_PHONE);
        this.peter = this.model.createObject(PETER_ID);
        final XField petersPhone = this.peter.createField(PHONE_ID);
    ...
    public void testFailureDoubleRemove() {
        ...
        final long petersPhoneRev = this.peter.getField(PHONE_ID).getRevisionNumber();

        final XTransactionBuilder tb = new XTransactionBuilder(this.model.getAddress());
        final XAddress modelAddr = this.model.getAddress();
        tb.removeObject(modelAddr, peterRev, PETER_ID);
        tb.addObject(modelAddr, XCommand.SAFE_STATE_BOUND, PETER_ID);
        final XAddress peterAddr = this.peter.getAddress();
        ...
        // listeners
        final HasChangedListener peterObjectListener = new HasChangedListener();
        final HasChangedListener peterFieldListener = new HasChangedListener();
        ...
        final HasChangedListener hc = HasChangedListener.listen(this.model);

        final long result = this.model.executeCommand(tb.build());
        ...
        final XField petersPhone = this.model.getObject(PETER_ID).getField(PHONE_ID);
        ...
        final XField peterAlias = this.model.getObject(PETER_ID).createField(ALIAS_ID);
    ...
    public void testModelTransactionFailure() {
        ...
        final long modelRev = this.model.getRevisionNumber();
        final long johnRev = this.john.getRevisionNumber();
        final long johnsPhoneRev = this.john.getField(PHONE_ID).getRevisionNumber();
        final long peterRev = this.model.getObject(PETER_ID).getRevisionNumber();
        final long petersPhoneRev = this.model.getObject(PETER_ID).getField(PHONE_ID).getRevisionNumber();

        final XTransactionBuilder tb = new XTransactionBuilder(this.model.getAddress());
        final XAddress modelAddr = this.model.getAddress();
        tb.removeObject(modelAddr, peterRev, PETER_ID);
        final XAddress johnAddr = this.john.getAddress();
        tb.addField(johnAddr, XCommand.SAFE_STATE_BOUND, ALIAS_ID);
        final XAddress aliasAddr = Base.resolveField(johnAddr, ALIAS_ID);
    ...
    public void testModelTransactionNoChange() {
        ...
        final long result = this.model.executeCommand(txn);
    ...
    public void testModelTransactionNoChangeRemoveAdd() {
        ...
        final XAddress johnAddr = this.john.getAddress();
        final XAddress peterAddr = this.peter.getAddress();
        final XAddress johnPhoneAddr = this.john.getField(PHONE_ID).getAddress();
        final XAddress peterPhoneAddr = this.peter.getField(PHONE_ID).getAddress();
    ...
    public void testModelTransactionSingleChange() {
        ...
        // record any changes and check that everything has been changed
        // correctly when the first event is executed
        final List<XEvent> received = ChangeRecorder.record(this.model);
        final List<XEvent> trans = ChangeRecorder.recordTransactions(this.model);
        ...
        final XEvent addJohnAlias = MemoryObjectEvent.createAddEvent(this.actorId, johnAddr, ALIAS_ID,
                modelRev, johnRev, true);

        final int x4 = received.indexOf(addJohnAlias);
    ...
    public void testModelTransactionSuccess() {
        ...
        final XAddress peterAddr = Base.resolveObject(modelAddr, PETER_ID);
        final XAddress peterPhoneAddr = Base.resolveField(peterAddr, PHONE_ID);
        final XEvent removePeterPhoneValue = MemoryReversibleFieldEvent.createRemoveEvent(this.actorId,
                peterPhoneAddr, PETER_PHONE, modelRev, peterRev, peterPhoneRev, true, true);
        final XEvent removePeterPhone = MemoryObjectEvent.createRemoveEvent(this.actorId, peterAddr,
                PHONE_ID, modelRev, peterRev, peterPhoneRev, true, true);
        final XEvent removePeter = MemoryModelEvent.createRemoveEvent(this.actorId, modelAddr, PETER_ID,
                modelRev, peterRev, true, false);

        final int x1 = received.indexOf(removePeterPhoneValue);
        assertTrue(x1 >= 0);
        final int x2 = received.indexOf(removePeterPhone);
        assertTrue(x2 >= 0);
        final int x3 = received.indexOf(removePeter);
        assertTrue(x3 >= 0);
        ...
        final XEvent addJohnAliasValue = MemoryReversibleFieldEvent.createAddEvent(this.actorId,
                aliasAddr, JOHN_ALIAS, modelRev, johnRev, johnPhoneRev, true);

        final int x5 = received.indexOf(addJohnAliasValue);
        assertEquals(4, x5);
        ...
        final XTransactionEvent te = (XTransactionEvent)trans.get(0);
    ...
    public void testObjectTransactionFailure() {
        ...
        final XField johnPhone = this.john.getField(PHONE_ID);

        final XTransactionBuilder tb = new XTransactionBuilder(this.john.getAddress());
        ...
        final HasChangedListener phoneListener = new HasChangedListener();
        johnPhone.addListenerForFieldEvents(phoneListener);

        final long result = this.john.executeCommand(tb.build());
        ...
        final XField phone = this.john.getField(PHONE_ID);
        assertNotNull(phone);
        ...
        final XValue newPhone = XV.toValue("0-NEW-PHONE");
    ...
    public void testObjectTransactionNoChange() {
        ...
        final XAddress johnAddr = this.john.getAddress();
        final XTransactionBuilder tb = new XTransactionBuilder(johnAddr);
        // should succeed and add john/alias
        tb.addField(johnAddr, XCommand.SAFE_STATE_BOUND, ALIAS_ID);
        // should succeed and do nothing (john/alias is already there)
        tb.addField(johnAddr, XCommand.FORCED, ALIAS_ID);
    ...
    public void testObjectTransactionSingleChange() {
        ...
        // correctly when the first event is executed
        final List<XEvent> received = ChangeRecorder.record(this.model);
        final List<XEvent> receivedFromObject = ChangeRecorder.record(this.john);
        final List<XEvent> trans = ChangeRecorder.recordTransactions(this.model);
        final List<XEvent> transFromObject = ChangeRecorder.recordTransactions(this.john);
    ...
    public void testObjectTransactionSuccess() {
        ...
        final long result = this.john.executeCommand(txn);
        ...
        final int x1 = received.indexOf(addJohnAlias);
        assertTrue(x1 >= 0);

        /* check events received for setting john/alias */

        final XEvent addJohnAliasValue = MemoryFieldEvent.createAddEvent(this.actorId, aliasAddr,
                JOHN_ALIAS, modelRev, johnRev, johnPhoneRev, true);

        final int x2 = received.indexOf(addJohnAliasValue);
        assertTrue(x2 >= 0);
