...
abstract public class AbstractSecureStoreReadMethodsTest extends AbstractStoreReadMethodsTest {
	...
	protected XID getCorrectUser() {
		...
		if(!this.acm.isAuthenticated(actorId, this.getCorrectUserPasswordHash())) {
			this.authenticationDb.setPasswordHash(actorId, HashUtils
			        .getXydraPasswordHash(this.correctPass));
		}
		...
		this.acm.getAuthorisationManager().getGroupDatabase().addToGroup(actorId,
		...
		XAddress repoAddress = XX.toAddress(this.store.getXydraStoreAdmin().getRepositoryId(),
		        null, null, null);
		assertTrue("admin group is allows to write repo", this.acm.getAuthorisationManager()
		        .getAuthorisationDatabase().getAccessDefinition(
		                XGroupDatabase.ADMINISTRATOR_GROUP_ID, repoAddress, XA.ACCESS_WRITE)
		        .isAllowed());
		assertTrue("admin group can write repo", this.acm.getAuthorisationManager().canWrite(
		        XGroupDatabase.ADMINISTRATOR_GROUP_ID, repoAddress));
		assertTrue("admin group can read repo", this.acm.getAuthorisationManager().canRead(
		        XGroupDatabase.ADMINISTRATOR_GROUP_ID, repoAddress));
		assertTrue("admin group can read model1", this.acm.getAuthorisationManager().canRead(
		        XGroupDatabase.ADMINISTRATOR_GROUP_ID, model1address));
