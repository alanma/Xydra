...
public abstract class AbstractStoreQuotaExceptionTest {
	...
	private static GetWithAddressRequest[] requestsForRandomModel() {
		return new GetWithAddressRequest[] { new GetWithAddressRequest(Base.toAddress(
		        Base.createUniqueId(), Base.createUniqueId(), Base.createUniqueId(), null)) };
	}
	...
	public void testExecuteCommandsQuotaException() {
		...
		for(long l = 0; l < this.bfQuota + 1; l++) {
			...
			final XCommand[] commands = new XCommand[] { getCommandFactory().createAddModelCommand(
			        Base.toId("data"), Base.createUniqueId(), true) };
