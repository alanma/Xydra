...
public class EventDeltaTest {
	...
	private XModel applyLocalChangesToEventDelta(EventDelta eventDelta) {
		...
		while(localEventIterator.hasNext()) {
			...
			boolean thing = localEvent.getTarget().equals(
			        XX.toAddress(localEvent.getChangedEntity().getRepository(), localEvent
			                .getChangedEntity().getModel(), XX.toId("claudia"), XX.toId("car")));
