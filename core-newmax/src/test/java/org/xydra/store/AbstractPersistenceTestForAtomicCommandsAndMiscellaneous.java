...
public abstract class AbstractPersistenceTestForAtomicCommandsAndMiscellaneous {
	...
	private void testRevisionSafeFieldCommands(XAddress modelAddress, XId objectId, XId fieldId,
	        ...
	        XObjectCommand addFieldAgainCom) {
		long result;
		// the field to whom we will add and change values
		XWritableField field;
		...
		/*
		 * test successful change value field command TODO find out right
		 * revNumber :-)
		 */
		XFieldCommand successfulChangeValueCom = this.comFactory.createSafeChangeValueCommand(
		        fieldAddress, 2, newValue);
		...
		/*
		 * test successful remove value command TODO find out right revNumber
		 */
	...
	private XObjectCommand testRevisionSafeObjectCommands(XAddress modelAddress, XId objectId,
		...
		XObjectCommand failingAddFieldCom = this.comFactory.createSafeAddFieldCommand(
		        objectAddress, fieldId);
		result = this.persistence.executeCommand(this.actorId, failingAddFieldCom);
		assertFalse(XCommandUtils.success(result));
		 ...
		 */
		XObjectCommand successfulAddFieldCom = this.comFactory.createSafeAddFieldCommand(
		        objectAddress, fieldId);
		result = this.persistence.executeCommand(this.actorId, successfulAddFieldCom);
		assertTrue(XCommandUtils.success(result));
		this.persistence.getModelSnapshot(new GetWithAddressRequest(modelAddress)).getObject(
		        objectId);
		 ...
		 */
		// TODO find out which is the right revision number :-)
	 ...
	 * @return
	 */
	private XModelCommand testRevisionSafeModelCommands(XAddress modelAddress, XId objectId,
	 ...
	 * @return
	 */
	private XCommand testRevisionSafeRepositoryCommands(XId modelId, XAddress modelAddress) {
