...
import org.xydra.core.model.impl.memory.UniqueEntityEvents;
 ...
 * 
 */
public class NewSyncer_Max {
    
    private static final Logger log = LoggerFactory.getLogger(NewSyncer_Max.class);
     ...
     * @param syncRev
     */
    public NewSyncer_Max(XydraStore store, IMemoryModel modelWithListeners,
    ...
    private class ServerCallback implements
        ...
        public void onFailure(Throwable exception) {
            NewSyncer_Max.this.onServerFailure(exception);
        }
        
        @Override
        public void onSuccess(Pair<BatchedResult<Long>[],BatchedResult<XEvent[]>[]> pair) {
            NewSyncer_Max.this.onServerSuccess(pair);
        }
    ...
    private void continueSync(XEvent[] serverEvents) {
        log.debug("event mapping to decide which local __cmds__ to consider succesful");
        /* event mapping */
        ...
        log.debug("***** Computing uniqueEvents from " + serverEvents.length + " server events");
        UniqueEntityEvents remote = new UniqueEntityEvents();
        for(XEvent serverEvent : serverEvents) {
            // FIXME
            log.debug(">>> Server event: " + serverEvent);
            remote.putEvent(serverEvent);
        }
        
        UniqueEntityEvents local = new UniqueEntityEvents();
        Iterator<ISyncLogEntry> localChanges = this.syncLog.getLocalChanges();
        while(localChanges.hasNext()) {
            ...
            local.putEvent(localEvent);
        }
        
        UniqueEntityEvents merged = UniqueEntityEvents.merge(local, remote);
        ...
        // change model state revision numbers
        NewSyncer_Max.applyEntityRevisionsToModel(serverEvents, this.modelState);
        ...
        // eventDelta.sendChangeEvents(this.root,
        // this.modelWithListeners.getAddress(),
    ...
    // */
    // @Deprecated
    // private void continueSync_OLD(XEvent[] serverEvents) {
    ...
    // log.debug(">>> Server event: " + serverEvent);
    //
    // eventDelta.addRemoteEvent(serverEvent);
    // }
    // Iterator<ISyncLogEntry> localChanges = this.syncLog.getLocalChanges();
    // while(localChanges.hasNext()) {
    // ISyncLogEntry localSyncLogEntry = localChanges.next();
    // XEvent localEvent = localSyncLogEntry.getEvent();
    // log.debug("<<< Local event: " + localEvent);
    // eventDelta.addLocalEvent(localEvent,
    // // this.syncRev,
    // this.syncLog);
    // }
    //
    ...
    // // choose your event mapper here
    // IEventMapper eventMapper = new UnorderedEventMapper();
    // IMappingResult mapping = eventMapper.mapEvents(this.syncLog,
    // serverEvents);
    //
    // // send sync events, let app see state before sync
    // for(Pair<XEvent,ISyncLogEntry> p : mapping.getMapped()) {
