...
public class XSynchronizer {
    ...
    /**
     * @param syncRev the syncRev of the entity on which the command (ac) was
     *            executed (its target)
     * @param i which command of a batch this is, index in localChanges
     * @param ac to be executed on the server
     * @return
     */
    private static XAtomicCommand fixAtomicCommand(long syncRev, int i, XAtomicCommand ac) {
        ...
        assert i > 0;
        assert ac.getRevisionNumber() > syncRev;
        ...
        XyAssert.xyAssert(rev < XCommand.RELATIVE_REV + i, "cmd.rev=%s i=%s syncRev=%s",
                ac.getRevisionNumber(), i, syncRev);
    ...
    private void doSynchronize(XSynchronizationCallback sc, boolean isFirst) {
        ...
        do {
            ...
            for(int i = 0; i < changes.length; i++) {
                if(!changes[i].isApplied()) {
                    if(!newChanges.isEmpty()
                            && (!XI.equals(actorId, changes[i].getActor()) || !XI.equals(
                                    passwordHash, changes[i].getPasswordHash()))) {
                        break;
                    }
            ...
            if(!newChanges.isEmpty()) {
                ...
                this.store.executeCommandsAndGetEvents(actorId, passwordHash, commands,
