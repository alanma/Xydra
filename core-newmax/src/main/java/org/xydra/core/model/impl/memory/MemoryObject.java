...
public class MemoryObject extends AbstractMOFEntity implements IMemoryObject, XObject,
     ...
     * @param objectState
     */
    public MemoryObject(MemoryModel father, MemoryEventQueue queue, XRevWritableObject objectState) {
     ...
     */
    private MemoryObject(MemoryModel father, XId actorId, String passwordHash,
            XAddress objectAddress, XRevWritableObject state, XChangeLogState log,
            boolean createObject) {
        super(father == null ? new Root() : father.getRoot(), createObject || state != null);
        this.father = father;
    ...
    @Override
    public void setRevisionNumber(long newRevision) {
        MemoryObject.this.state.setRevisionNumber(newRevision);
    }
