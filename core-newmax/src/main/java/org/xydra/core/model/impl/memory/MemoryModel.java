...
Serializable {
    ...
    public long executeCommand(XCommand command, XLocalChangeCallback callback) {
        return executeCommandWithActor(command, getSessionActor(), getSessionPassword(), callback);
    }
    ...
    private long executeModelCommand(XModelCommand command, XLocalChangeCallback callback) {
        assert command != null;
        return executeModelCommandWithActor(command, getSessionActor(), getSessionPassword(),
                callback);
    }
     ...
     */
    private MemoryModel(IMemoryRepository father, XId actorId, String passwordHash,
            XAddress modelAddress, XRevWritableModel modelState, XChangeLogState changeLogState,
            boolean createModel) {
        super(new Root(), createModel || modelState != null);
        this.father = father;
        if(father != null) {
     ...
     * @param log @CanBeNull
     */
    public MemoryModel(MemoryRepository father, XId actorId, String passwordHash,
     ...
     */
    static MemoryModel createNonExistantModel(MemoryRepository father, XId actorId,
            String passwordHash, XId repoId, XId modelId) {
        assert actorId != null;
        assert modelId != null;
        
        XAddress modelAddress = XX.resolveModel(father.getId(), modelId);
        MemoryModel nonExistingModel = new MemoryModel(father, actorId, passwordHash, modelAddress,
                null, null, false);
    ...
    public MemoryModel(IMemoryRepository father, XId actorId, String passwordHash,
        ...
        super(new Root(), modelState != null);
