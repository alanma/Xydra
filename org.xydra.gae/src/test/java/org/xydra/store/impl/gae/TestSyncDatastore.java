...
public class TestSyncDatastore {
	...
	public void testBatchPut() {
		...
		list.add(XGae.get().datastore()
				.createEntity(XGae.get().datastore().createKey("kind1", "key2")));
		list.add(XGae.get().datastore()
				.createEntity(XGae.get().datastore().createKey("kind1", "key3")));
		...
		final Map<SKey, SEntity> map = XGae
				...
				.getEntities(
						...
						XGae.get().datastore().createKey("kind1", "key1"), XGae.get().datastore()
