...
public class Utils {
    ...
    public static Set<XId> findChildren(XAddress address) {
        ...
        log.debug(XGaeDebugHelper.dataGet(XGae.get().datastore().sync().getDatastoreName(), "query-children:"
                + address.toURI(), DebugFormatter.format(childIds), Timing.Now));
        return childIds;
    }
    ...
    public static Iterator<XAddress> findModelAdresses() {
        ...
        SPreparedQuery preparedQuery = XGae.get().datastore().sync().prepareRangeQuery(
                KeyStructure.KIND_XCHANGE, true, null, "1");
        final Iterator<SEntity> it = preparedQuery.asIterable().iterator();
        
        return new TransformingIterator<SEntity,XAddress>(it, new ITransformer<SEntity,XAddress>() {
