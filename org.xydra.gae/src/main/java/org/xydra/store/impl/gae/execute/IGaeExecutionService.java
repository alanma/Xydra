...
public interface IGaeExecutionService {
	 ...
	 * 
	 * @param command
	 *            The command to execute. (can be a {@link XTransaction})
	 * @param actorId
	 *            The actor to log in the resulting event.
	 ...
	 *             if we came too close to the timeout while executing the
	 *             command. A caller may catch this exception and try again, but
	 *             doing so may just result in a timeout from GAE if
	 *             TIME_CRITICAL is set to more than half the GAE timeout.
	 * 
