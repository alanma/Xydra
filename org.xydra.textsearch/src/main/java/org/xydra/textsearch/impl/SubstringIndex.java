...
/**
 * Does not index anything, despite the name.
 * 
 * @author xamde
 * 
 * @param <V>
...
public class SubstringIndex<V> extends AbstractTokenIndex<V> {
    ...
    protected Collection<String> generateTokenFragments(String token) {
        return generateSubstrings(token, this.minSubstringLength, this.maxSubStringLength);
    }
    
    /**
     * TODO returns also prefix-substrings?
     * 
     * @param t
     ...
     */
    private static Set<String> generateSubstrings(String t, int minimalSubStringLength,
            int maximalSubstringLength) {
